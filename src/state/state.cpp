...
int State::DispatchTerminalResponse(String command, RequestId request_id,
                                    VariantMap& payload) {
  if (command == "presonalization") {
    // if (!PersonalizationResponse(request_id, payload)) {
    logger.error("Failed to handle personalization");
    return -1;
    // }
    return 0;
  }
  logger.error("Unknown Type=%s", command.c_str());
  return -2;
}

void State::Loop() {
  if (auto state = std::get_if<tag::Authorize>(tag_state_.get())) {
    if (auto new_state = tag::StateLoop(*state)) {
      tag_state_ = std::make_shared<tag::State>(new_state.value());
    }
  } else if (auto state = std::get_if<tag::Personalize>(tag_state_.get())) {
    if (auto new_state = tag::StateLoop(*state, *static_cast<CloudRequest*>(this))) {
      tag_state_ = std::make_shared<tag::State>(new_state.value());
    }
  }
}
...
//   HexStringToBytes<16>(payload.get("authorization")); if (!authorization_key)
//   return tl::unexpected(ErrorType::kMalformedResponse); auto reserved1_key =
//   HexStringToBytes<16>(payload.get("reserved1")); if (!reserved1_key) return
//   tl::unexpected(ErrorType::kMalformedResponse); auto reserved2_key =
//   HexStringToBytes<16>(payload.get("reserved2")); if (!reserved2_key) return
//   tl::unexpected(ErrorType::kMalformedResponse);
...
void State::OnBlankNtag(std::array<std::byte, 7> uid) {
  tag_state_ = std::make_shared<tag::State>(tag::Personalize{
      .tag_uid = uid,
      // .state =
      // std::make_shared<tag::personalize::State>(tag::personalize::Wait{
      //     .timeout = millis() + 3000,
      // })
  });
}
