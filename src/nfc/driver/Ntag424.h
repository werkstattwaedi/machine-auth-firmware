...
class Ntag424 {
 ...
 public:
  ...
  // on a new tag).
  DNA_StatusCode DNA_Plain_ChangeFileSettings(
  ...
  // optional Use this function only if you want to use SDM and know what you
  // are doing.
  DNA_StatusCode DNA_Plain_ChangeFileSettings(DNA_File file, byte* sendData,
                                              byte sendDataLen);

  // Reads UID when random ID is not enabled.
  // If random ID is enabled, reads random ID. To read the true ID in that case,
  // use DNA_Full_GetCardUID. This is only a wrapper function. It gets
  // uid.uidByte from MFRC522 lib. void DNA_Plain_GetCardUID(byte* backUID_7B);

  DNA_StatusCode DNA_Plain_GetFileSettings(DNA_File file, byte* backRespData,
                                           byte* backRespLen);

  DNA_StatusCode DNA_Plain_GetFileSettings_AccessRights(
      DNA_File file, byte* backReadAccess, byte* backWriteAccess,
      byte* backReadWriteAccess, byte* backChangeAccess);

  DNA_StatusCode DNA_Plain_GetFileSettings_CommMode(DNA_File file,
                                                    DNA_CommMode* backCommMode);

  DNA_StatusCode DNA_Plain_GetFileSettings_SDM(DNA_File file,
                                               bool* backSDMEnabled);

  // Writes to backRespData 28 or 29 bytes according to tables 54, 56 and 58
  // from NT4H2421Gx (NTAG 424 DNA) datasheet: VendorID, HWType, HWSubType,
  // HWMajorVersion, HWMinorVersion, HWStorageSize, HWProtocol, VendorID,
  // SWType, SWSubType, SWMajorVersion, SWMinorVersion, SWStorageSize,
  // SWProtocol, UID(7), BatchNo(4), BatchNo/FabKey, FabKey/CWProd, YearProd,
  // [FabKeyID] Note: arguments in brackets are optional; SW1 and SW2 are not
  // included in backRespData
  DNA_StatusCode DNA_Plain_GetVersion(byte* backRespData, byte* backRespLen);

  // Data read in 59 B blocks
  DNA_StatusCode DNA_Plain_ISOReadBinary(DNA_File file, uint16_t length,
                                         byte offset, byte* backReadData,
  ...
  DNA_StatusCode DNA_Plain_ISOUpdateBinary(DNA_File file, uint16_t length,
                                           byte offset, byte* sendData);

  // Data read in 59 B blocks
  DNA_StatusCode DNA_Plain_ReadData(DNA_File file, uint16_t length, byte offset,
                                    byte* backReadData, uint16_t* backReadLen);
  ...
  DNA_StatusCode DNA_Plain_WriteData(DNA_File file, uint16_t length,
                                     byte offset, byte* sendData);
  ...
  // (NTAG 424 DNA) datasheet: FileType, FileOption, AccessRights(2),
  // FileSize(3), [SDMOptions], [SDMAccessRights(2)], [UIDOffset(3)],
  // [SDMReadCtrOffset(3)], [PICCDataOffset(3)], [PICCDataOffset(3)],
  // [SDMENCOffset(3)], [SDMENCLength(3)], [SDMMACOffset(3)],
  // [SDMReadCtrLimit(3)] Note: arguments in brackets are optional; SW1 and SW2
  // are not included in backRespData
  DNA_StatusCode DNA_Mac_GetFileSettings(DNA_File file, byte* backRespData,
                                         byte* backRespLen);

  DNA_StatusCode DNA_Mac_GetFileSettings_AccessRights(DNA_File file,
                                                      byte* backReadAccess,
                                                      byte* backWriteAccess,
                                                      byte* backReadWriteAccess,
                                                      byte* backChangeAccess);

  DNA_StatusCode DNA_Mac_GetFileSettings_CommMode(DNA_File file,
                                                  DNA_CommMode* backCommMode);

  DNA_StatusCode DNA_Mac_GetFileSettings_SDM(DNA_File file,
                                             bool* backSDMEnabled);
  ...
  DNA_StatusCode DNA_Mac_ReadData(DNA_File file, uint16_t length, byte offset,
                                  byte* backReadData, uint16_t* backReadLen);

  // Data written in 41 B blocks
  DNA_StatusCode DNA_Mac_WriteData(DNA_File file, uint16_t length, byte offset,
                                   byte* sendData);
  ...
  // Use this function if you do not need to use SDM (SDM is disabled by default
  // on a new tag).
  DNA_StatusCode DNA_Full_ChangeFileSettings(
      DNA_File file, DNA_CommMode commMode, byte readAccess, byte writeAccess,
      byte readWriteAccess, byte changeAccess, byte SDMEnabled);
  ...
  // optional Use this function only if you want to use SDM and know what you
  // are doing.
  DNA_StatusCode DNA_Full_ChangeFileSettings(DNA_File file, byte* sendData,
                                             byte sendDataLen);

  DNA_StatusCode DNA_Full_ChangeKey(byte keyNumber, byte* oldKey, byte* newKey,
                                    byte newKeyVersion);
  ...
  // Reads SDMReadCtr. This command works if you enable SDM on your own, but it
  // is not implemented in this library currently.
  DNA_StatusCode DNA_Full_GetFileCounters(DNA_File file,
                                          uint32_t* backSDMReadCtr);

  // Data read in 47 B blocks
  DNA_StatusCode DNA_Full_ReadData(DNA_File file, uint16_t length, byte offset,
                                   byte* backReadData, uint16_t* backReadLen);

  // Response from Full_Read_Sig is 74 B, but MFRC522 can only handle 61 B
  // responses, so it can't be implemented. If at least 64 B were available, it
  // would be possible. See DNA_Plain_Read_Sig
  // DNA_StatusCode DNA_Full_Read_Sig - not implemented (MFRC522 hardware
  // limitation)

  DNA_StatusCode DNA_Full_SetConfiguration(byte* sendData, byte sendDataLen);

  DNA_StatusCode DNA_Full_SetConfiguration_FailedCtrOption(
  ...
  DNA_StatusCode DNA_Full_SetConfiguration_PDCap2(bool turnLRPModeON_Permanent,
                                                  byte PDCap2_5, byte PDCap2_6);
  ...
  DNA_StatusCode DNA_Full_SetConfiguration_StrongBackModulation(
      bool StrongBackModulation);

  // Data written in 31 B blocks
  DNA_StatusCode DNA_Full_WriteData(DNA_File file, uint16_t length, byte offset,
                                    byte* sendData);
 ...
 protected:
  ...
  DNA_StatusCode DNA_AuthenticateEV2First_Part1(byte keyNumber, byte* backData,
                                                byte* backLen);
  DNA_StatusCode DNA_AuthenticateEV2First_Part2(byte* inData, byte* backData,
                                                byte* backLen);
  DNA_StatusCode DNA_AuthenticateEV2NonFirst_Part1(byte keyNumber,
                                                   byte* backData,
                                                   byte* backLen);
  DNA_StatusCode DNA_AuthenticateEV2NonFirst_Part2(byte* inData, byte* backData,
                                                   byte* backLen);

  DNA_StatusCode DNA_Plain_GetVersion_native(byte Cmd, byte expectedSV2,
                                             byte* backRespData,
                                             byte* backRespLen);
  DNA_StatusCode DNA_Plain_ISOReadBinary_native(DNA_File file, byte length,
                                                byte offset, byte* backReadData,
                                                byte* backReadLen);
  DNA_StatusCode DNA_Plain_ISOUpdateBinary_native(DNA_File file, byte length,
                                                  byte offset, byte* sendData);
  DNA_StatusCode DNA_Plain_ReadData_native(DNA_File file, byte length,
                                           byte offset, byte* backReadData,
                                           byte* backReadLen);
  DNA_StatusCode DNA_Plain_WriteData_native(DNA_File file, byte length,
                                            byte offset, byte* sendData);

  DNA_StatusCode DNA_Mac_GetVersion_native(byte Cmd, byte expectedSV2,
                                           byte* backRespData,
                                           byte* backRespLen);
  DNA_StatusCode DNA_Mac_ReadData_native(DNA_File file, byte length,
                                         byte offset, byte* backReadData,
                                         byte* backReadLen);
  DNA_StatusCode DNA_Mac_WriteData_native(DNA_File file, byte length,
                                          byte offset, byte* sendData);

  DNA_StatusCode DNA_Full_ReadData_native(DNA_File file, byte length,
                                          byte offset, byte* backReadData,
                                          byte* backReadLen);
  DNA_StatusCode DNA_Full_WriteData_native(DNA_File file, byte length,
                                           byte offset, byte* sendData);

  DNA_StatusCode DNA_CheckResponseCMACt(byte* responseCMACt);
  DNA_StatusCode DNA_CheckResponseCMACtWithData(byte* data, byte dataLen,
                                                byte* responseCMACt);
  ...
  void DNA_CalculateCMACtNoData(byte Cmd, byte* CmdHeader, byte CmdHeaderLen,
                                byte* backCMACt);
  void DNA_CalculateCRC32NK(byte* message16, byte* backCRC);
  void DNA_CalculateDataEncAndCMACt(byte Cmd, byte* dataToEnc,
